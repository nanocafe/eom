import { DocumentMagnifyingGlassIcon, HomeIcon } from '@heroicons/react/20/solid'
                ...
                <Link href={`/snapshots/${year}/${month}`}>
                  <a className="flex">
                    <div className='w-full flex space-x-4 py-2'>
                      <div className='w-32 font-semibold'>{year}, {monthName(Number(year), Number(month))}</div>
                      <div className='flex-1 text-center'><span className='font-semibold'>{competitions[year][month]}</span> guesses</div>
                      <div><DocumentMagnifyingGlassIcon className='w-5 h-5' /></div>
                    </div>
                  </a>
                </Link>
              </li>
